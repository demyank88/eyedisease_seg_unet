...
class Model2eye():
    def __init__(self,config,mode='normal'):
        ...
        self.timestr = datetime.datetime.today().strftime("%Y-%m-%d%H-%M-%S")
    ...
    def save(self,epoch):
        ...
        self.model.save(self.config.checkpoint_dir + 'epoch_{}_{}'.format(self.timestr,epoch))
        # self.model.save_weights(os.path.join(self.config.checkpoint_dir,"segmentation_epoch_{}.h5".format(epoch)))

    def restore(self, N=None):
        path2load_model = os.path.join("outputs_2nd/checkpoints/epoch_2020-10-2307-24-23_{}".format(N))
        # self.model.compile(optimizer='rmsprop', loss='binary_crossentropy', metrics=['accuracy'])
